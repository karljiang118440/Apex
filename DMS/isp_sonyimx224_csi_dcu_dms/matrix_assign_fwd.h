...
// this code is here to perform an unrolled version of the matrix_assign() function
    template < typename DEST, typename SRC1, long NR, long NC,
    long R = 0, long C = 0, bool base_case = (R==NR) >
    struct matrix_unroll_helper
    {
        inline static void go ( DEST& dest, const SRC1& src)
        {
            dest(R,C) = src(R,C);
            matrix_unroll_helper<DEST,SRC1,NR,NC, R + (C+1)/NC,  (C+1)%NC>::go(dest,src);
        }
    };

    template < typename DEST, typename SRC1, long NR, long NC, long R, long C >
    struct matrix_unroll_helper<DEST,SRC1,NR,NC,R,C,true>
    { inline static void go ( DEST& , const SRC1& ) {} };
    ...
    !*/
    {
        COMPILE_TIME_ASSERT(SRC1::NR*SRC1::NC != 0);
        matrix_unroll_helper<DEST,SRC1, SRC1::NR, SRC1::NC>::go(dest,src);
    }
